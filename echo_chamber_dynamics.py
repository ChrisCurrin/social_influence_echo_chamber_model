...
class EchoChamberDynamics(object):
    ...
    def final_exports(self, t):
        ...
        self.social_media.message_df.to_csv(
            os.path.join(self.data_dir + "/data", "messages.csv.xz")
        )
        self.export_gexf(t)

    def evolve(self, t_max, mu, p, q, rewiring_methods, nudge=False):
        for t in range(t_max):
            ...
            self.agents[user_id].screen_diversity = analysis.screen_diversity(
                screen.content.values, bins=10
            )
            ...
            if nudge:
                ...
                nudge_strength, sample_size, *nudge_fn = nudge
                all_current_opinions = np.array(self.opinion_data[-1])

                if nudge_fn:
                    nudge_value = nudge_fn[0](all_current_opinions, sample_size)
                else:
                    nudge_value = np.sqrt(sample_size) * (
                        sample_means(all_current_opinions, sample_size)
                        - np.mean(all_current_opinions)
                    )
                self.agents[user_id].opinion = (
                    self.agents[user_id].opinion
                    + np.sqrt(1) * nudge_strength * nudge_value[0]
                )

            # rewiring (q)
            if np.random.random() < q:
                unfollow_id, follow_id = self.agents[user_id].decide_to_rewire(
                    self.social_media, rewiring_methods
                )
